...
public final class InjectAdapterProcessor extends AbstractProcessor {
  ...
  private void generateInjectAdapter(TypeElement type, ExecutableElement constructor,
    ...
    TypeSpec.Builder result = TypeSpec.classBuilder(adapterClassName.simpleName())
        ...
        .superclass(parameterizedType(Binding.class, injectedClassName))
        .addJavadoc("$L", bindingTypeDocs(injectableType(type.asType()), isAbstract,
            injectMembers, dependent).toString());

    if (constructor != null) {
      result.addSuperinterface(parameterizedType(Provider.class, injectedClassName));
    }
    if (injectMembers) {
      result.addSuperinterface(parameterizedType(MembersInjector.class, injectedClassName));
    }
  ...
  private FieldSpec memberBindingField(boolean disambiguateFields, Element field) {
    return FieldSpec.builder(
        parameterizedType(Binding.class, injectableType(field.asType())),
  ...
  private FieldSpec parameterBindingField(boolean disambiguateFields, VariableElement parameter) {
    return FieldSpec.builder(
            parameterizedType(Binding.class, injectableType(parameter.asType())),
            parameterName(disambiguateFields, parameter),
            PRIVATE).build();
  }

  private FieldSpec supertypeBindingField(TypeMirror supertype) {
    return FieldSpec.builder(parameterizedType(Binding.class, injectableType(supertype)),
        "supertype", PRIVATE).build();
  }
    ...
    if (supertype != null) {
      result.addStatement(
          "$N = ($T) linker.requestBinding($S, $T.class, getClass().getClassLoader()"
              + ", false, true)",
          "supertype",
          parameterizedType(Binding.class, injectableType(supertype)),
